package com.example.android_2_bmi;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        height = (EditText)findViewById(R.id.et1); // 取得身高物件
        weight = (EditText)findViewById(R.id.et2); // 取得體重物件
        ...
        submit.setOnClickListener(new Button.OnClickListener() {
            public void onClick(View arg0) {
                ...
                {
                    float fheight = Float.parseFloat(height.getEditableText().toString());      // 取得 身高輸入值
                    float fweight = Float.parseFloat(weight.getEditableText().toString());     // 取得 體重輸入值
                    float fresult;                                     // BMI值 計算結果
                    TextView result = (TextView)findViewById(R.id.tv3);// 取得 顯示結果 物件
                    fheight = fheight/100; // 計算BMI
                    fheight = fheight*fheight;  // 計算BMI

                    NumberFormat nf = NumberFormat.getInstance();   // 數字格式
                    nf.setMaximumFractionDigits(2);                 // 限制小數第二位
                    fresult = fweight/fheight;                                // 計算BMI
                    result.setText(nf.format(fweight/fheight) +"");           // 顯示BMI計算結果
